#!/usr/bin/env python3
"""Update plmc to the latest commit on debbiemarkslab/plmc.
...
Run from the repository root; rewrites packages/plmc/default.nix in place.
"""
...
REPO = "plmc"
NIX = Path("packages/plmc/default.nix")
...
def main() -> None:
    """Bump plmc to the newest upstream commit, if any."""
    text = NIX.read_text()
    ...
    if sha == current_rev.group(1):
        print(f"plmc already at {sha[:9]} ({date})")
        return

    sri = prefetch_hash(sha)
    text = replace_field(text, "version", f"0-unstable-{date}")
    text = replace_field(text, "rev", sha)
    text = replace_field(text, "hash", sri)
    NIX.write_text(text)
    print(f"plmc -> 0-unstable-{date} ({sha[:9]})")
