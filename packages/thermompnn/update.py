#!/usr/bin/env python3
"""Update ThermoMPNN to the latest commit on Kuhlman-Lab/ThermoMPNN.

The repo carries no tags/releases, so track its default-branch HEAD by commit
date. Each bump stays pinned to a specific commit.

Run from the repository root; rewrites packages/thermompnn/default.nix in place.
"""
...
OWNER = "Kuhlman-Lab"
REPO = "ThermoMPNN"
NIX = Path("packages/thermompnn/default.nix")
...
def main() -> None:
    """Bump ThermoMPNN to the newest upstream commit, if any."""
    text = NIX.read_text()
    ...
    if sha == current_rev.group(1):
        print(f"thermompnn already at {sha[:9]} ({date})")
        return

    sri = prefetch_hash(sha)
    text = replace_field(text, "version", f"1.0-unstable-{date}")
    text = replace_field(text, "rev", sha)
    text = replace_field(text, "hash", sri)
    NIX.write_text(text)
    print(f"thermompnn -> 1.0-unstable-{date} ({sha[:9]})")
